...
// the right place
template <typename Fifo> class FrameAssembler {
  ...
  public:
    void assemble() {
        std::vector<int64_t> frame_numbers(fifos.size()); // template frame numbers
        std::vector<typename Fifo::value_type> frames(fifos.size());
    ...
    void push_back(Fifo *ptr) { fifos.push_back(ptr); }
    void stop() {
        stopped = true;
